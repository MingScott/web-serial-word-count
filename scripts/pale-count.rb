...
chap = Chapter.new "https://palewebserial.wordpress.com/2020/05/05/blood-run-cold-0-0/"
...
CSV.open("data/pale.csv","wb") do |csv|
	csv << ["date","wordcount","chapter","work"]
	for ii in 0..data["date"].length-1
		csv << [data["date"][ii],data["wordcount"][ii],data["chapter"][ii],"Pale"]
	end
end