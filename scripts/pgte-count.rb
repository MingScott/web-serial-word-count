...
chap = Chapter.new "https://practicalguidetoevil.wordpress.com/2015/03/25/prologue/"
...
CSV.open("data/pgte.csv","wb") do |csv|
	csv << ["date","wordcount","chapter","work"]
	for ii in 0..data["date"].length-1
		csv << [data["date"][ii],data["wordcount"][ii],data["chapter"][ii],"PGTE"]
	end
end