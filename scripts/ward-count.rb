...
chap = Chapter.new "https://www.parahumans.net/2017/09/11/daybreak-1-1/"
...
CSV.open("data/ward.csv","wb") do |csv|
	csv << ["date","wordcount","chapter","work"]
	for ii in 0..data["date"].length-1
		csv << [data["date"][ii],data["wordcount"][ii],data["chapter"][ii],"Ward"]
	end
end